...
from parsers import (parse_data_from_email,
                     get_variant_number_from_travis_log)
...
def get_data_from_email(imap, message_mail):
    ...
    if status == 'OK':
        ...
        if message['Subject'] == 'OS':
            group_name, student_name, repo_url = parse_data_from_email(
                message)
        else:
            # не тот заголовок
            status = 'NO'
    ...
    if number_lab in (1, 2):
        ...
        if check_runs is not None:
            for check_run in check_runs:
                if check_run['conclusion'] == "success" and (
                        check_run['status'] == "completed"):
                    date_completed = max(check_run['completed_at'],
                                         date_completed)
                else:
                    date_completed = None
                    break
        ...
        if date_completed is not None:
            ...
            travis_client = TravisClient(
                github_token=github_api.get_token(),
                private=github_api.repo_is_private())
            job_log = travis_client.get_first_job_log(
                build_id=external_id)
            number_variant = get_variant_number_from_travis_log(
                job_log, number_lab)
    elif number_lab == 3:
...
def main():
    ...
    spreadsheet = SpreadSheet(
        json_keyfile='google_service_account.json',
    ...
    for current_index_email in list_email:
        ...
        status, student_email, group_name, student_name, repository_url = get_data_from_email(
            imap, current_index_email)
        if status != 'OK':
        ...
        print('Email  ', student_email, 'Группа ', group_name, 'ФИО ',
              student_name, 'Ссылка ', repository_url)
        ...
        osn_ch2 = _l1[1]
        osn_ch3 = _l1[2]
        # print('ОСНАВНАЯ ЧАСТЬ ССЫЛКИ ', osn_ch1, osn_ch2) # 'https://github.com/suai-2019/os'
        name = _l[1]
        print()
        # номео лабораторной
        task_id = int(name.split('-')[1].replace('task', ''))
        print('Organization ', organization, ' Никнэйм ', name,
              ' номер лабораторной №', task_id)
        ...
        if worksheet is None:
            # Видимо направильный номер группы.
            server_mail.send(student_email,
                             'Нет группы ' + group_name,
                             'Ошибка группы')
            continue
        ...
        if student_name not in list_fio:
            ...
            server_mail.send(
                student_email,
                'Нет такого студента ' + student_name + 'в группе ' + group_name,
                'Ошибка ФИО'
            )
            continue

        # print('2 студент в группе ',group_name, ' ФИО ',
        # student_name, ' номер (0..n)=',
        # list_fio.index(student_name)-2)
        number_row = list_fio.index(student_name) + 3
        # конец работы с google

        # разбор ссылки
        """вычисление № лабораторной лаботы """
        if ('https://github.com/suai' == osn_ch1) and (
                'os' == osn_ch2) and ('2019/os' == osn_ch3):
            # МОЖЕТ БЫТЬ ТУТ СТОИТ СДЕЛАТЬ ПРОВЕРКУ НА СУЩЕСТВОВАНИЕ ЛАБОРАТОРНОЙ РАБОТЫ?
            # print ('3 лабораторная работа ', task_id, ' никнэйм студента:', name)
            number_col = int(task_id - 1) * 3 + 13
            # запись ссылки на лабу в таблицу гугла
            worksheet.update_cell(number_row, number_col,
                                  repository_url)
            # print('ячейка row=', number_row, ' col=', number_col,
            # ' на странице ', group_name,
            # ' GOOGLE таблицы, успешно обновлена на ',
            # repository_url)
            ...
            try:
                # 'v2.7w5hnu6pmhkm1rpfesuq'
                appveyor_token = os.environ['APPVEYOR_TOKEN']
                appveyor_client = AppVeyorClient(
                    api_token=appveyor_token,
                    org_name=services['appveyor']['organization'],
                    project_name=github_api.get_repo_name())
                is_green, variant_number, completion_date = get_checks_status(
                    task_id, github_api, appveyor_client)
            ...
            if not is_green:
                server_mail.send(
                    student_email,
                    'Не выполнена лабораторная работа №' + str(
                        task_id),
                    'Ошибка лабораторной работы')
                continue

            print('GITHUB LABS ', completion_date, ' ВАРИАНТ ',
                  variant_number, ' в гугле ', number_row - 2)
            need_number = whats_variants(task_id, number_row - 2)

            if (completion_date is not None) and (
                    variant_number == need_number):
                # запись даты выполнения лабы
                worksheet.update_cell(number_row, number_col + 1,
                                      completion_date)
                print('ячейка row=', number_row, ' col=', number_col,
                      ' на странице ', group_name,
                      ' GOOGLE таблицы, успешно обновлена на ',
            ...
            # ERROR вызов процедуры отправки письма с этим сообщением
            server_mail.send(student_email,
                             'Проверьте вашу ссылку на репозиторий',
                             'Ошибка основной части ссылки')
            # конец разбор ссылки
